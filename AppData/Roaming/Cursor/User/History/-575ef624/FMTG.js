...
function stopTour() {
  ...
  const startFloatingChaos = () => {
    if (currentDelay < 30) {
      floatingInterval = setInterval(() => {
        // Calculate time since chaos started
        const timeSinceStart = Date.now() - chaosStartTime;
        const secondsSinceStart = timeSinceStart / 1000;
        
        // EXPONENTIAL MULTIPLICATION: More messages as time passes!
        // Start with 1 message every 100ms
        // After 5 seconds: 2 messages every 100ms
        // After 10 seconds: 4 messages every 100ms
        // After 15 seconds: 8 messages every 100ms
        // etc...
        const multiplier = Math.pow(2, Math.floor(secondsSinceStart / 5)); // Double every 5 seconds
        const messageCount = Math.min(multiplier, 20); // Cap at 20 messages per interval
        
        // Create multiple messages at once!
        for (let i = 0; i < messageCount; i++) {
          createFloatingThankYou();
        }
      }, 100); // Still every 100ms, but creating multiple messages
    }
  };
  ...
  const speedCheckInterval = setInterval(() => {
    if (currentDelay < 30 && !floatingInterval) {
      chaosStartTime = Date.now(); // Reset timer when chaos starts
      startFloatingChaos();
    }
  }, 100);
