/* Languages Page JavaScript - Clean Rebuild */

let isSpeaking = false;
let currentIndex = 0;
let speakTimeout;

// Simple languages data
const languages = [
...
function createLanguageGrid() {
    ...
    languages.forEach((language, index) => {
        ...
        // Click to speak individual language
        item.onclick = () => speakLanguage(language, index);
        
        grid.appendChild(item);
    });
}

// Toggle speaking (start/stop)
function toggleSpeaking() {
    console.log('toggleSpeaking called, isSpeaking:', isSpeaking);
    if (isSpeaking) {
        stopSpeaking();
    } else {
        startSpeaking();
...
function startSpeaking() {
    ...
    if (button) {
        ...
        console.log('Button updated to Stop Speaking');
    }
...
// Speak next language
function speakNext() {
    if (!isSpeaking || currentIndex >= languages.length) {
        stopSpeaking();
        return;
    }
    
    const language = languages[currentIndex];
    speakLanguage(language, currentIndex);
    
    currentIndex++;
    
    // Schedule next language after 1 second
...
function updateLanguageInfo(info) {
    const infoElement = document.getElementById('currentLanguage');
    if (infoElement) {
