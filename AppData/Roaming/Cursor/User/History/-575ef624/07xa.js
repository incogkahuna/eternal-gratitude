...
function stopTour() {
  ...
  let currentDelay = 200; // Start fast
  let speedMultiplier = 0.8; // Each second, multiply by 0.8 (gets faster)
  ...
  playNextLanguage = function() {
    ...
    speak(text, langCode, () => {
      if (isPlaying) {
        ...
        setTimeout(playNextLanguage, Math.max(currentDelay, 10)); // Minimum 10ms
  ...
  const showNextMessage = () => {
    if (messageIndex < messages.length) {
      ...
      setTimeout(() => {
        message.style.animation = 'slideOutRight 0.2s ease forwards';
        setTimeout(() => message.remove(), 200);
      }, Math.max(1000 - (messageIndex * 100), 200));
  ...
  let chaosInterval = setInterval(() => {
    if (currentDelay < 50) { // When it's really fast
      // Add random color flashes to the background
      document.body.style.animation = 'chaosFlash 0.1s ease infinite';
    }
  }, 1000);
