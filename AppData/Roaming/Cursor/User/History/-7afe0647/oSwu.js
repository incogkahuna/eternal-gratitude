...
function handleScroll() {
  ...
  // If scrolled near bottom (within 800px), generate more (larger trigger zone)
  if (scrollTop + clientHeight >= scrollHeight - 800) {
    generateMessages(15); // Generate more messages at once
  }
...
function init() {
  ...
  // Also generate more every few seconds if user isn't scrolling (much faster!)
  setInterval(() => {
    if (messageCount < 200) { // Allow more messages
      generateMessages(8); // Generate more at once
    }
  }, 1500); // Much faster interval (was 3000ms)
}
